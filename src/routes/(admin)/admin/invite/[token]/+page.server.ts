 ...
 *
 *   GET  /admin/invite/{token}   →  show signup form pre-filled with email
 *   POST /admin/invite/{token}   →  create user via Better Auth, set the
...
export const actions: Actions = {
  default: async ({ request, params, platform }) => {
    ...
    try {
      ...
      // through /admin/signup with the same address before accepting).
    ...
    throw redirect(302, "/admin/login?invited=1");
  },
};
