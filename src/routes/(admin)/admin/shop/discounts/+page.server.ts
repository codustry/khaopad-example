...
export const actions: Actions = {
  create: async ({ request, locals, platform }) => {
    ...
    // D3: 'code' (typed at checkout) vs 'automatic' (applies to every
    // qualifying checkout; best-of arbitration in checkout/start).
    const method = String(fd.get("method") ?? "code") as "code" | "automatic";
    ...
    if (!["code", "automatic"].includes(method)) {
      return fail(400, { error: "Invalid discount method" });
    }
    // Automatic discounts have no customer-facing code — a sentinel is
    // generated by createDiscount (the column is NOT NULL + UNIQUE).
    if (method === "code" && !code) {
    ...
    // The v3.5 free_shipping creation block is lifted: #158 (v3.13)
    // shipped server-priced shipping, so free-shipping discounts now
    // apply a real amount at checkout (discountIsFreeShipping path).
    ...
    try {
      const id = await createDiscount(env.DB, {
        code: method === "automatic" ? undefined : code,
        method,
        kind,
        valueSatang,
      ...
      await logAudit(env.DB, locals.user.id, "discount.created", id, {
        code,
        method,
      });
      return {
        ...
        message:
          method === "automatic"
            ? "Created automatic discount"
            : `Created code ${code.toUpperCase()}`,
      };
    } catch (err) {
      return fail(400, {
